 ...
 * <li><p>If the advanced {@code encodeSegments()} function with 6 arguments or the {@code
 *   makeSegmentsOptimally()} function was called, then increase the maxVersion argument if it was
 *   less than {@link QrCode#MAX_VERSION}. (This advice does not apply to the other factory
 *   functions because they search all versions up to {@code qr.QrCode.MAX_VERSION}.)</p></li>
 * <li><p>Split the text data into better or optimal segments in order to reduce the number of bits
 *   required. (See {@link makeSegmentsOptimally(CharSequence,QrCode.Ecc,int,int)
 *   qr.QrSegmentAdvanced.makeSegmentsOptimally()}.)</p></li>
 * <li><p>Change the text or binary data to be shorter.</p></li>
 * <li><p>Change the text to fit the character set of a particular segment mode (e.g.
 *   alphanumeric).</p></li>
...
internal object Helper {
  internal fun panicIf(exp: Boolean, msg: String) {
    if (exp) error("unexpected error: $msg")
  }

  internal infix fun Int.panicIfNotIn(range: IntRange) {
    if (this !in range) error("$this is not in range $range")
  }

  internal infix fun Int.panicIfNot(other: Int) {
    if (this != other) error("$this is not $other")
  }
}
