...
// @formatter:off
private fun QrCode.Ecc.codeWordsPerBlock(version: QrCode.Version) =
  when (this) {
    QrCode.Ecc.LOW ->
      byteArrayOf(
        7,
...
private fun QrCode.Ecc.numErrorCorrectionBlocks(version: QrCode.Version) =
  when (this) {
    QrCode.Ecc.LOW ->
      byteArrayOf(
        1,
...
// after
// all function modules are excluded. This includes remainder bits, so it might not be a multiple of
// 8.
private val QrCode.Version.numRawDataModules
  get() =
    intArrayOf(
      208,
      359,
...
private class QrCodeBuilder(
  ...
  fun drawFormatBits(msk: QrCode.Mask) {
    ...
    var rem = data
    for (i in 0..<10) rem = (rem shl 1) xor ((rem ushr 9) * 0x537)
    ...
    for (i in 9..<15) setFunctionModule(14 - i, 8, bits.getBit(i))

    // Draw second copy
    for (i in 0..<8) setFunctionModule(size - 1 - i, 8, bits.getBit(i))
    for (i in 8..<15) setFunctionModule(8, size - 15 + i, bits.getBit(i))
  ...
  fun drawVersion() {
    ...
    for (i in 0..<12) rem = (rem shl 1) xor ((rem ushr 11) * 0x1F25)
  ...
  fun drawAlignmentPattern(x: Int, y: Int) {
    for (dy in -2..2) for (dx in -2..2) setFunctionModule(
      x + dx,
      y + dy,
      abs(dx).coerceAtLeast(abs(dy)) != 1,
    )
  }
...
internal fun buildQrCode(
  ...
  val builder = QrCodeBuilder(version = version, errorCorrectionLevel = errorCorrectionLevel)
  ...
  val mask =
    QrCode.Mask.entries.minBy { mask ->
...
private fun reedSolomonComputeRemainder(data: ByteArray, divisor: ByteArray): ByteArray {
  ...
  for (b in data) { // Polynomial division
    ...
    for (i in result.indices) result[i] =
      result[i] xor reedSolomonMultiply(divisor[i].toInt() and 0xFF, factor).toByte()
  }
  return result
}

// Returns the product of the two given field elements modulo GF(2^8/0x11D). The arguments and
// result
// are unsigned 8-bit integers. This could be implemented as a lookup table of 256*256 entries of
// uint8.
...
private fun QuadraticBoolMatrix.getPenaltyScore2x2Pattern(): Int =
  sequence { (x, y), color ->
      x < size - 1 &&
        y < size - 1 &&
        color == this[x + 1, y] &&
...
private fun QuadraticBoolMatrix.getPenaltyScoreFinderLikePatterns(): Int {
  ...
  fun finderPenaltyAddHistory(currentRunLength: Int, runHistory: IntArray) {
    ...
    if (runHistory[0] == 0) result += size // Add light border to initial run
