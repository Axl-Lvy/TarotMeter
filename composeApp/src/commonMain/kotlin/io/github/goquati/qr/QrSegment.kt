 ...
 * immutable.
 *
 * <p>The mid-level way to create a segment is to take the payload data and call a static factory
 * function such as {@link QrSegment#makeNumeric(CharSequence)}. The low-level way to create a
 ...
 *
 * <p>This segment class imposes no length restrictions, but QR Codes have restrictions. Even in the
 * most favorable conditions, a QR Code can only hold 7089 characters of data. Any segment longer
 * than this is meaningless for the purpose of generating QR Codes. This class can represent kanji
 * mode segments, but provides no help in encoding them
 * - see {@link QrSegmentAdvanced} for full kanji support.</p>
 */
@ConsistentCopyVisibility
public data class QrSegment
internal constructor(
   ...
   * numeric/alphanumeric/kanji mode, bytes for byte mode, and 0 for ECI mode. Always zero or
   * positive. Not the same as the data's bit length.
   */
  val numChars: Int,
  ...
  public companion object {
    ...
    internal fun getTotalBits(segs: List<QrSegment>, version: QrCode.Version): Int =
      segs
        .sumOf { seg ->
    ...
    /**
     * Adds a segment representing the specified binary data encoded in byte mode. All input byte
    ...
    /**
     * Returns a segment representing the specified binary data encoded in byte mode. All input byte
     * arrays are acceptable.
     *
     * <p>Any text string can be converted to UTF-8 bytes ({@code
     * s.getBytes(StandardCharsets.UTF_8)}) and encoded as a byte mode segment.</p>
    ...
    public fun makeBytes(data: ByteArray): QrSegment {
      val bb = buildList { for (b in data) appendBits((b.toInt() and 0xFF), 8) }
     ...
     */
    public fun MutableList<QrSegment>.addNumeric(digits: CharSequence): Boolean =
      add(makeNumeric(digits))

    /**
     * Returns a segment representing the specified string of decimal digits encoded in numeric
     * mode.
     *
     * @param digits the text, with only digits from 0 to 9 allowed
    ...
    /**
     * Adds a segment representing the specified text string encoded in alphanumeric mode. The
     ...
     */
    public fun MutableList<QrSegment>.addAlphanumeric(text: CharSequence): Boolean =
      add(makeAlphanumeric(text))

    /**
     * Returns a segment representing the specified text string encoded in alphanumeric mode. The
     * characters allowed are: 0 to 9, A to Z (uppercase only), space, dollar, percent, asterisk,
     * plus, hyphen, period, slash, colon.
     *
     * @param text the text, with only certain characters allowed
    ...
    /**
     * Adds a list of zero or more segments to represent the specified Unicode text string. The
     * result may use various segment modes and switch modes to optimize the length of the bit
     * stream.
     *
     * @param text the text to be encoded, which can be any Unicode string
     * @return true because the list is always modified as the result of this operation.
     */
    public fun MutableList<QrSegment>.addSegment(text: CharSequence): Boolean =
      add(makeSegment(text))

    /**
     * Returns a list of zero or more segments to represent the specified Unicode text string. The
     * result may use various segment modes and switch modes to optimize the length of the bit
     * stream.
     *
     * @param text the text to be encoded, which can be any Unicode string
     ...
     */
    public fun makeSegment(text: CharSequence): QrSegment =
      when {
    ...
    /**
     * Adds a segment representing an Extended Channel Interpretation (ECI) designator with the
     * specified assignment value.
     *
    ...
    /**
     * Returns a segment representing an Extended Channel Interpretation (ECI) designator with the
     * specified assignment value.
     *
     * @param assignVal the ECI assignment number (see the AIM ECI specification)
     * @return a segment containing the data
    ...
    /**
     * Tests whether the specified string can be encoded as a segment in numeric mode. A string is
     * encodable iff each character is in the range 0 to 9.
     */
    private fun CharSequence.isNumeric() = all { it in NUMERIC_CHARSET }

    /**
     * Tests whether the specified string can be encoded as a segment in alphanumeric mode. A string
     * is encodable iff each character is in the following set: 0 to 9, A to Z (uppercase only),
