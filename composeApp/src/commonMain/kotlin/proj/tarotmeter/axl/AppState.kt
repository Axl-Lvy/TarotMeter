...
class AppState {
  ...
  init {
    ...
    if (players.isEmpty()) {
      repeat(5) { i -> players.add(Player(id = "P${i+1}", name = "Player ${i+1}")) }
    }
  }
  ...
  fun totalScore(game: Game, playerId: String): Int = game.rounds.sumOf { it.scores[playerId] ?: 0 }

  private fun generateId(prefix: String): String = buildString {
    append(prefix)
    append("-")
